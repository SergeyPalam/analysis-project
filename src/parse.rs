...
/// Конструктор [UnquoteNoEsc]
fn unquote_noesc() -> UnquoteNoEsc {
    UnquoteNoEsc
}
...
impl Parser for QuotedTag {
    type Dest = ();
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ()> {
        let (remaining, candidate) = unquote_noesc().parse(input)?;
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
fn _all3<A0: Parser, A1: Parser, A2: Parser>(a0: A0, a1: A1, a2: A2) -> All<(A0,A1,A2)> {
    All { parser: (a0, a1, a2) }
}
...
/// (в Rust нет чего-то, вроде variadic templates из C++)
fn _all4<A0: Parser, A1: Parser, A2: Parser, A3: Parser>(a0: A0, a1: A1, a2: A2, a3: A3) -> All<(A0,A1,A2,A3)> {
...
impl Parsable for AuthData {
    ...
    fn parser () -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| AuthData{data: authdata})
    }
}

/// Конструкция 'либо-либо'
enum _Either<Left,Right> {
    Left(Left),
    Right(Right),
}
...
mod test {
    ...
    #[test]
    fn test_quote() {
        assert_eq!(Quote.parse(r#"411"#).unwrap().1, r#""411""#);
        assert_eq!(Quote.parse(r#"4\11""#).unwrap().1, r#""4\\11\"""#);
    }

    #[test]
    fn test_do_unquote_non_escaped() {
        assert_eq!(UnquoteNoEsc.parse(r#""411""#), Ok(("", "411".to_string())));
        assert_eq!(UnquoteNoEsc.parse(r#" "411""#), Err(()));
        assert_eq!(UnquoteNoEsc.parse(r#"411"#.into()), Err(()));
    }
